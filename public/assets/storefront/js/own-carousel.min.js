...
function responsive() {
  let windowWidth = window.innerWidth,
    flag = !1,
    crsContainer = document.querySelectorAll(".own-carousel__container"),
    containerArray = Array.from(crsContainer);
  containerArray.forEach((item) => {
    ...
    flag ||
      ...
      (item.gapWidth =
        (100 - item.itemPerRow * item.itemWidth) / (item.itemPerRow - 1) || 0),
...
(Object.prototype.ownCarousel = function (options) {
  const {
    ...
    autoplay: autoplay = 0,
    stopAutoplayWhenHover: stopAutoplayWhenHover = !0,
    nav: nav = !1,
    isRTL: isRTL = !1,
  } = options;
  if (
    ...
    (this.step =
      this.imgWidth + (this.gapWidth / this.itemWidth) * this.imgWidth),
    ...
    for (let i = 0; i < this.itemPerRow; i++) {
      let cloneNode = this.carouselItem[i].cloneNode(!0);
      this.carousel.insertAdjacentElement("beforeend", cloneNode);
    }
    let count = 0;
    for (; count != this.itemPerRow; ) {
      let cloneNode = this.carouselItem[this.numberOfItem - 1].cloneNode(!0);
      this.carousel.insertAdjacentElement("afterbegin", cloneNode), count++;
    }
  }
  if (
    ((this.translateSlide = () => {
    ...
    (this.resetSlide = (step) => {
      this.index + step < 1 && (this.index = this.numberOfItem + 1),
    ...
    (this.moveSlide = (step) => {
      if (((this.carousel.style.transition = "all 0.25s"), loop))
        this.index + step < 1 ||
        this.index + step >= this.numberOfItem + this.itemPerRow
          ? this.resetSlide(step)
          : ((this.index += step), this.translateSlide());
      else {
        if (
          this.index + step < 0 ||
          this.index + step > this.numberOfItem - this.itemPerRow
    ...
    let firstPos = (currentPos = 0),
      dragStartHandle = (e) => {
        (this.carousel.style.transition = "none"),
          ...
          (firstPos = currentPos),
          autoplay && (clearInterval(intervalId), clearTimeout(timeoutId));
      },
      dragHandle = (e) => {
        let currentMove = parseFloat(this.carousel.style.transform.slice(12)),
          currentIndex = -currentMove / this.step,
          x = "touchmove" === e.type ? e.touches[0].clientX : e.clientX,
          distanceMoved = x - currentPos;
        loop
          ? currentIndex <= 0
            ? ((this.index = this.numberOfItem), this.translateSlide())
            : currentIndex >= this.numberOfItem + this.itemPerRow
            ? ((this.index = this.itemPerRow), this.translateSlide())
            : (this.carousel.style.transform = `translate3d(${
    ...
    this.checkIndex = (currentMove) => {
      ...
      for (; temp >= this.step; ) temp -= this.step;
      return (temp > 50 && firstPos - currentPos >= 0) ||
        (this.step - temp < 50 && firstPos - currentPos <= 0)
        ? Math.ceil(currentMove / this.step)
        : Math.floor(currentMove / this.step);
    };
    let dragEndHandle = (e) => {
      "touchend" === e.type
  ...
  mouseWheel &&
    this.carouselOuter.addEventListener("wheel", (e) => {
      e.preventDefault(), e.deltaY > 0 ? this.moveSlide(-1) : this.moveSlide(1);
    }),
    autoplay &&
      ((timeoutId = setTimeout(() => {
        intervalId = setInterval(
