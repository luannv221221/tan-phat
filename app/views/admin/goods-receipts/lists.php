...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h3 class="card-title"><i class="fas fa-dolly-flatbed mr-2"></i>{{$page_name}}</h3>
        <div class="card-tools">
            @if (route('admin/'.$routeBase.'/add'))
            <a href="{{_WEB_URL.'/admin/'.$routeBase.'/add'}}" class="btn btn-primary btn-sm">
                <i class="fas fa-plus mr-1"></i> Lập phiếu nhập
            </a>
            @endif
        </div>
    </div>
        ...
        <table class="table table-hover text-nowrap mb-0">
            <thead>
                <tr>
                    ...
                    <th>Loại / Kho</th>
                    <th>Nhà cung cấp</th>
                    <th style="width:15%" class="text-right">Giá trị</th>
            ...
            @if (!empty($dataList))
                ...
                <tr>
                    <td><code>{{$item['receipt_no']}}</code></td>
                    <td>{{$item['receipt_date']}}</td>
                    <td>
                        <span class="badge badge-light border">{{$types[$item['receipt_type']] ?? $item['receipt_type']}}</span>
                        <span class="text-muted small d-block">{{$item['warehouse_name']}}</span>
                    </td>
                    <td>{{!empty($item['partner_full']) ? $item['partner_full'] : (!empty($item['partner_name']) ? $item['partner_name'] : '—')}}</td>
                ...
                @endforeach
            @else
                <tr><td colspan="7" class="text-center text-muted py-4"><i class="fas fa-inbox fa-2x d-block mb-2"></i> Chưa có phiếu nhập nào</td></tr>
            @endif
            </tbody>
        </table>
    </div>
