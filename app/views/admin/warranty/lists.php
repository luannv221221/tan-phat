...
<div class="card card-outline card-primary">
    <div class="card-header">
        ...
        <div class="card-tools">
            ...
            <a href="{{_WEB_URL.'/admin/'.$routeBase.'/add?loai=bao_hanh'}}" class="btn btn-primary btn-sm"><i class="fas fa-tools mr-1"></i> Lập phiếu bảo hành</a>
            <a href="{{_WEB_URL.'/admin/'.$routeBase.'/add?loai=bao_tri'}}" class="btn btn-info btn-sm"><i class="fas fa-oil-can mr-1"></i> Lập phiếu bảo trì</a>
            @endif
        </div>
    </div>
    ...
    <div class="card-body border-bottom">
        <form method="get" class="form-row align-items-end">
            <div class="form-group col-md-2 mb-2">
                ...
                <select name="loai" class="form-control form-control-sm">
                    <option value="">— Cả hai —</option>
                    @foreach ($loais as $k => $label)
                    <option value="{{$k}}" {{$filterLoai===$k?'selected':''}}>{{$label}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group col-md-2 mb-2">
            ...
            <div class="form-group col-md-1 mb-2 text-nowrap">
    ...
    <div class="card-body table-responsive p-0">
        <table class="table table-hover text-nowrap mb-0">
            ...
            @if (!empty($dataList))
                ...
                <?php $__bt = (($item['loai'] ?? '') === 'bao_tri'); ?>
                <tr>
                    <td>
                        <code>{{$item['request_no']}}</code>
                    ...
                    <td>
                        @if (!empty($item['bien_so']))
                            ...
                            {!! !empty($item['so_km']) ? '<span class="text-muted small d-block">'.number_format($item['so_km'], 0, ',', '.').' km</span>' : '' !!}
                        @else
