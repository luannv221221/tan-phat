...
<div class="card card-outline card-warning">
    ...
    <div class="card-body table-responsive p-0">
        <table class="table table-hover text-nowrap mb-0">
            <thead><tr><th style="width:130px">Số phiếu</th><th style="width:110px">Ngày</th><th>Kho</th><th style="width:14%" class="text-right">Thừa</th><th style="width:14%" class="text-right">Thiếu</th><th style="width:120px" class="text-center">Trạng thái</th><th style="width:70px" class="text-center">Xem</th></tr></thead>
            <tbody>
            @if (!empty($dataList))
                @foreach ($dataList as $item)
                <tr>
                    <td><code>{{$item['take_no']}}</code></td>
                    <td>{{$item['take_date']}}</td>
                    <td>{{$item['warehouse_name']}}</td>
                    <td class="text-right text-success">{{number_format((float)$item['surplus_value'],0,',','.')}}</td>
                    <td class="text-right text-danger">{{number_format((float)$item['shortage_value'],0,',','.')}}</td>
                    <td class="text-center">{!! $item['status']==1 ? '<span class="badge badge-primary">Đã chốt</span>' : '<span class="badge badge-secondary">Nháp</span>' !!}</td>
                ...
                @endforeach
            @else
                <tr><td colspan="7" class="text-center text-muted py-4"><i class="fas fa-inbox fa-2x d-block mb-2"></i> Chưa có phiếu kiểm kê nào</td></tr>
            @endif
            </tbody>
        </table>
    </div>
