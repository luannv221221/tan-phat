...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h3 class="card-title"><i class="fas fa-calendar-alt mr-2"></i>{{$page_name}}</h3>
        ...
        <table class="table table-hover text-nowrap mb-0">
            <thead>
                <tr>
                    ...
                    <th>Model</th>
                    <th style="width:16%" class="text-center">Đời (năm)</th>
            ...
            @if (!empty($dataList))
                ...
                <tr>
                    ...
                    <td class="font-weight-bold">{{$item['model_name']}}</td>
                    <td class="text-center">
                        <span class="badge badge-info">{{$item['year_from']}}{{!empty($item['year_to']) ? ' – '.$item['year_to'] : ' – nay'}}</span>
                    </td>
                    <td>{!! !empty($item['name']) ? e($item['name']) : '<span class="text-muted">—</span>' !!}</td>
                    <td class="text-center">
                    ...
                    <td class="text-center">
                        ...
                        <a onclick="return confirm('Bạn có chắc chắn muốn xoá đời xe này?')" href="{{_WEB_URL.'/admin/'.$routeBase.'/delete/'.$item['id']}}" class="btn btn-danger btn-sm" title="Xoá"><i class="fas fa-trash"></i></a>
                        @endif
                    </td>
                </tr>
                @endforeach
