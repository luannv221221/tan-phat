...
if (!empty($old['line_part']) && is_array($old['line_part'])){
    foreach ($old['line_part'] as $i => $p){ $initRows[] = ['part_id' => (int) $p, 'qty' => isset($old['line_qty'][$i]) ? $old['line_qty'][$i] : '', 'note' => isset($old['line_note'][$i]) ? $old['line_note'][$i] : '']; }
} else {
    foreach ($items as $it){ $initRows[] = ['part_id' => (int) $it['part_id'], 'qty' => rtrim(rtrim((string) $it['quantity'], '0'), '.'), 'note' => $it['note']]; }
}
...
<div class="card {{$posted?'card-outline card-primary':'card-outline card-secondary'}}">
    <div class="card-header">
        <h3 class="card-title"><i class="fas fa-exchange-alt mr-2"></i>Điều chuyển <code>{{$item['transfer_no']}}</code></h3>
...
@if ($posted)
    <div class="card"><div class="card-body">
        <dl class="row mb-0">
            <dt class="col-sm-3">Ngày</dt><dd class="col-sm-9">{{$item['transfer_date']}}</dd>
            <dt class="col-sm-3">Lý do</dt><dd class="col-sm-9">{{!empty($item['reason'])?$item['reason']:'—'}}</dd>
            <dt class="col-sm-3">Tổng giá trị chuyển</dt><dd class="col-sm-9 font-weight-bold">{{number_format((float)$item['total_value'],0,',','.')}} ₫</dd>
        </dl>
    </div></div>
    ...
    <div class="card"><div class="card-body">
        ...
        <a href="{{_WEB_URL.'/admin/'.$routeBase.'/unpost/'.$item['id']}}" onclick="return confirm('Huỷ ghi sổ sẽ hoàn tồn về kho nguồn. Tiếp tục?')" class="btn btn-warning"><i class="fas fa-unlock mr-1"></i> Huỷ ghi sổ</a>
        @endif
        ...
        <div class="card"><div class="card-body">
            <div class="form-row">
                <div class="form-group col-md-3">
                    ...
                    <input type="date" name="transfer_date" class="form-control" value="{{$sel('transfer_date')}}"/>
                ...
                <div class="form-group col-md-4">
                    ...
                    <select name="from_warehouse_id" class="form-control">
                        ...
                        <option value="{{$w['id']}}" {{$sel('from_warehouse_id')==$w['id']?'selected':''}}>{{$w['code'].' - '.$w['name']}}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-4">
                    ...
                    <select name="to_warehouse_id" class="form-control">
                        <option value="">— Chọn kho —</option>
                        @foreach ($warehouses as $w)
                        <option value="{{$w['id']}}" {{$sel('to_warehouse_id')==$w['id']?'selected':''}}>{{$w['code'].' - '.$w['name']}}</option>
                        @endforeach
                    </select>
                    {!! !empty($errors['to_warehouse_id'])?'<small class="text-danger">'.e($errors['to_warehouse_id']).'</small>':false !!}
                </div>
            </div>
            <div class="form-group mb-0"><label>Lý do</label><input type="text" name="reason" class="form-control" value="{{$sel('reason')}}"/></div>
        </div></div>
