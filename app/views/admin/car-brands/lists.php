        ...
        <table class="table table-hover text-nowrap mb-0">
            <thead>
                <tr>
                    ...
                    <th>Tên hãng</th>
                    <th style="width:18%">Đường dẫn (slug)</th>
            ...
            @if (!empty($dataList))
                ...
                <tr>
                    ...
                    <td class="text-center">
                        {!! !empty($item['logo']) ? '<img src="'.e(_WEB_URL.'/'.'public/assets/uploads/brands/'.$item['logo']).'" alt="logo" style="height:34px;max-width:70px;object-fit:contain">' : '<span class="text-muted">-</span>' !!}
                    </td>
                    <td class="font-weight-bold">{{$item['name']}}</td>
                    <td><code>{{$item['slug']}}</code></td>
                    <td>{{!empty($item['country']) ? $item['country'] : '—'}}</td>
                    ...
                    <td class="text-center">
                        ...
                        <a onclick="return confirm('Bạn có chắc chắn muốn xoá hãng này?')" href="{{_WEB_URL.'/admin/'.$routeBase.'/delete/'.$item['id']}}" class="btn btn-danger btn-sm" title="Xoá"><i class="fas fa-trash"></i></a>
                        @endif
                    </td>
                </tr>
                @endforeach
