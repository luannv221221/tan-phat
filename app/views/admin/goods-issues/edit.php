...
<div class="card {{$posted?'card-outline card-primary':'card-outline card-secondary'}}">
    <div class="card-header">
        <h3 class="card-title"><i class="fas fa-dolly mr-2"></i>{{$types[$item['issue_type']] ?? 'Phiếu xuất'}} — <code>{{$item['issue_no']}}</code></h3>
...
@if ($posted)
    <div class="card"><div class="card-body">
        <dl class="row mb-0">
            <dt class="col-sm-3">Ngày</dt><dd class="col-sm-9">{{$item['issue_date']}}</dd>
            ...
            <dt class="col-sm-3">Tổng giá vốn</dt><dd class="col-sm-9 font-weight-bold">{{number_format((float)$item['total_amount'],0,',','.')}} ₫</dd>
        </dl>
    </div></div>
    ...
    <div class="card"><div class="card-body">
        ...
        <a href="{{_WEB_URL.'/admin/'.$routeBase.'/unpost/'.$item['id']}}" onclick="return confirm('Huỷ ghi sổ sẽ hoàn lại tồn kho. Tiếp tục?')" class="btn btn-warning"><i class="fas fa-unlock mr-1"></i> Huỷ ghi sổ</a>
        @endif
...
@else
    <form action="{{_WEB_URL.'/admin/'.$routeBase.'/edit/'.$item['id']}}" method="post">
            ...
            <div class="form-row">
                <div class="form-group col-md-3">
                    ...
                    <select name="type" class="form-control">
                        ...
                        <option value="{{$k}}" {{$sel('issue_type')==$k?'selected':''}}>{{$label}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-3">
                    <label>Ngày <span class="text-danger">*</span></label>
                    <input type="date" name="issue_date" class="form-control" value="{{$sel('issue_date')}}"/>
                    {!! !empty($errors['issue_date'])?'<small class="text-danger">'.e($errors['issue_date']).'</small>':false !!}
                </div>
        ...
        <div class="card card-outline card-info">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-list-ol mr-2"></i>Dòng hàng xuất</h3>
