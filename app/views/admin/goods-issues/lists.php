...
<div class="card card-outline card-danger">
    <div class="card-header">
        <h3 class="card-title"><i class="fas fa-dolly mr-2"></i>{{$page_name}}</h3>
        <div class="card-tools">
            @if (route('admin/'.$routeBase.'/add'))
            <a href="{{_WEB_URL.'/admin/'.$routeBase.'/add'}}" class="btn btn-primary btn-sm">
                <i class="fas fa-plus mr-1"></i> Lập phiếu xuất
            </a>
            @endif
        </div>
    </div>
        ...
        <table class="table table-hover text-nowrap mb-0">
            <thead>
                <tr>
                    ...
                    <th>Loại / Kho</th>
                    <th>Đối tượng</th>
                    <th style="width:15%" class="text-right">Giá vốn</th>
            ...
            @if (!empty($dataList))
                ...
                <tr>
                    <td><code>{{$item['issue_no']}}</code></td>
                    <td>{{$item['issue_date']}}</td>
                    <td>
                        <span class="badge badge-light border">{{$types[$item['issue_type']] ?? $item['issue_type']}}</span>
                ...
                @endforeach
            @else
                <tr><td colspan="7" class="text-center text-muted py-4"><i class="fas fa-inbox fa-2x d-block mb-2"></i> Chưa có phiếu xuất nào</td></tr>
            @endif
            </tbody>
        </table>
    </div>
