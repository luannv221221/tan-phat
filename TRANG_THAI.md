...
- **Phân hệ CSKH — CSKH-2 BB giao nhận**: ✅ **XONG** (19/07, migration `000031`): biên bản giao nhận thiết bị bảo hành (BB **nhận** / **trả**, số `BBGN-`) + **bản in A4** ký giao/nhận. Dùng chung quyền `warranty`.
- **Phân hệ CSKH — web (nốt)**: ✅ **XONG** (19/07, migration `000033`/`000034`): **đăng ký bản tin** (`admin/newsletter`) + **hộp thư liên hệ** (`admin/contact-messages`, form `/lien-he`) + **nhắc bảo trì tự động** (`admin/nhac-bao-tri`, ngày hoàn tất + chu kỳ cấu hình, lọc quá hạn/cần nhắc, đánh dấu đã nhắc).
...
- **Phân hệ Bán hàng (SAL)**: ✅ **XONG lát cắt Bán hàng-1** (18/07, migration `000017`, `KHO_BAN_HANG_SPEC.md`): báo giá (trạng thái + chuyển thành hoá đơn) · hoá đơn bán (ghi sổ sinh 1 phiếu kế toán **Nợ131/Có511 + Nợ131/Có3331 + Nợ632/Có156** và trừ tồn) · công nợ khách (dùng lại `admin/debt`) · báo cáo bán hàng (doanh thu/giá vốn/lãi gộp theo khách + NV). **KT-6 ĐÃ KHÉP VÒNG.** ✅ **Nốt (19/07, migration `000035`)**: chiết khấu theo dòng + auto chiết khấu nhóm KH + **HĐĐT nội bộ** (phát hành số HĐ + xuất XML TT78/NĐ123, không nối NCC). Hoãn: hợp đồng (không cần), chiết khấu header, in PDF.
- **Phân hệ Kho — Kho-3**: ✅ **XONG** (19/07, migration `000030` + `000032`): **hàng tồn lâu** (`ton-kho-lau`, số ngày nằm kho + dải tuổi tồn) + **vị trí trong kho** (`warehouse-locations`, cây ≤5 cấp) + **biểu đồ biến động tồn** (`bien-dong-ton`, SVG nhập/xuất/tồn) + **bắt buộc chọn vị trí trên phiếu nhập** (select phụ thuộc kho, lưu `location_id`). Nhóm menu "Kho".
